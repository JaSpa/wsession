...
\newcommand\Target{} % {\tcboxmath[size=fbox,size=tight,arc=0ex,boxrule=0ex,colback=blue!20]}
\newcommand\Source{\tcboxmath[size=fbox,size=tight,arc=0ex,boxrule=0ex,colback=blue!20]}
...
\stCommand
  ...
  The {\ACSEND} command has a callback to obtain the message's payload
  from the application state $A$. As the payload type $T$ is just
  syntax, we translate it to an Agda type using the function
  {\stTypeInterpretationSignature} (see Section~\ref{sec:finite-nonbr-simple}). Similarly, the {\ACRECV} command has a
  ...
  A program is expressed as a value of type {\ACommand}. We implement
  the same functionality as our example in Listing~\ref{lst:traditional}, hence we choose
  {\AZ}, the integers, as the application state. The encoding relies
  heavily on the core idea of functional programming: functions
...
\stNegpCommand
  ...
  The interpreter takes a command, a suitable
  initial application state, and an untyped channel to compute an IO
...
\stExecutorSignature
  ...
  channel. A manual proof is not difficult as each command is
  implemented in few lines of code.
\end{description}
...
\item We introduce the callback approach to binary session
  ...
  programming.
%   We deploy it as a proof-of-concept specification in the
...
% For connoisseurs of pointfree definitions, we give a
...
The \AgdaFunction{push} function takes a {\ACommandStack} and a
...
all intervening continuations from the store. This case reveals a complication. The recursive
...
termination checker. To console it we mark the {\Aexecutor} function
with a termination pragma.
Using this pragma is a pragmatic choice as our intention is to
actually run this code or use it as a blueprint for a Haskell implementation. 

\subsection{What about proofs?}
\label{sec:what-about-proofs}

When it comes to proving linear handling of channels in the
interpreter, we plan to introduce additional structure and modify
{\Aexecutor} so that it always takes a single step 
...
the setting with loops as a value \AgdaFunction{cms} of type {\ACommandStack~$(\Asuc~n)$~$A$}
can serve as a continuation. The only change in the code of
{\Aexecutor} (besides the return type) is the case for {\ACONTINUE}, which
returns a continuation.
\rstAlternative
To restart a continuation, we simply invoke
{\Aexecutor} on \AgdaFunction{cms}~\Azero. 
\rstAlternativeExecutorRestart
With this approach, we can safely prove properties of {\Aexecutor}. To
actually run the code, we still need a potentially nonterminating loop to repeatedly restart the
...
\AgdaKeyword{variable}-feature to introduce typed
